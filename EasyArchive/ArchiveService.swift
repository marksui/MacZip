...
enum ArchiveServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .permissionDenied(path):
            return "EasyArchive does not have permission to use \(path)."
        case let .extractionFailed(reason):
            return "EasyArchive could not extract the archive. \(reason)"
        case let .compressionFailed(reason):
            return "EasyArchive could not create the zip file. \(reason)"
        case .unsupportedSelection:
...
struct ArchiveService {
    ...
    func compressItems(_ itemURLs: [URL], to outputFolder: URL) -> [ArchiveOperationResult] {
        do {
            try validateOutputFolder(outputFolder)
            ...
            try runProcess(
                ...
                arguments: ["-r", "-y", destinationURL.path] + relativePaths,
